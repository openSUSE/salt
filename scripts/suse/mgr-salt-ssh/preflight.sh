...
REPO_HOST=$1
CLIENT_REPOS_ROOT="https://${REPO_HOST}/pub/repositories"
...
if [ -x /usr/bin/dnf ]; then
    ...
    echo "Error: Unable to detect installer on the OS!" >&2
    exit 1
fi
...
if [ -x /usr/bin/wget ]; then
    ...
    echo "Error: To be able to download files, please install either 'wget' or 'curl'" >&2
    exit 2
fi
...
if [ -f "${VENV_FILE}" ]; then
    VENV_PKG_PATH=$(cat "${VENV_FILE}")
    echo "$VENV_PKG_PATH" >> /tmp/venv.log
    VENV_HASH=$(sha256sum < "${VENV_FILE}" | tr -d ' -')
    ...
    if [ "${VENV_HASH}" != "${PRE_VENV_HASH}" ]; then
        ...
        if [ -f "${VENV_PKG_FILE}" ]; then
            ...
            sed -i 's#^export VIRTUAL_ENV=.*#export VIRTUAL_ENV="/var/tmp/venv-salt-minion"#; s#^exec /usr/lib/venv-salt-minion/#/var/tmp/venv-salt-minion/#' bin/python
            popd > /dev/null
