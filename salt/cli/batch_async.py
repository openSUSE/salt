...
__SHARED_EVENT_CHANNELS = None


def _get_shared_event_channels(opts, io_loop):
    global __SHARED_EVENT_CHANNELS
    if __SHARED_EVENT_CHANNELS is None:
        __SHARED_EVENT_CHANNELS = SharedEventChannels(opts, io_loop)
    return __SHARED_EVENT_CHANNELS


def _destroy_unused_shared_event_channels():
    global __SHARED_EVENT_CHANNELS
    if __SHARED_EVENT_CHANNELS is not None and __SHARED_EVENT_CHANNELS.destroy_unused():
        __SHARED_EVENT_CHANNELS = None
...
class BatchAsync:
    ...
    def __init__(self, parent_opts, jid_gen, clear_load):
        ...
        self.event_channels = _get_shared_event_channels(parent_opts, self.io_loop).use(
            id(self)
        )
        if "gather_job_timeout" in clear_load["kwargs"]:
            ...
            clear_load["gather_job_timeout"] = self.event_channels.local_client.opts[
                "gather_job_timeout"
            ]
        ...
        self.opts = batch_get_opts(
            ...
            self.event_channels.local_client.opts,
            **clear_load,
        )
        ...
        self.ended = False
        self.event = self.event_channels.master_event
        self.scheduled = False

    def __set_event_handler(self):
        self.event_channels.subscribe(
    ...
    def check_find_job(self, batch_minions, jid):
        ...
        if not self.event:
            return
        self.event_channels.unsubscribe(jid, "find_job_return", id(self))
    ...
    def find_job(self, minions):
        ...
        try:
            ...
            ret = yield self.event_channels.local_client.run_job_async(
                not_done,
                "saltutil.find_job",
                [self.batch_jid],
                "list",
    ...
    def start(self):
        ...
        ping_return = yield self.event_channels.local_client.run_job_async(
            self.opts["tgt"],
            "test.ping",
            [],
    ...
    def start_batch(self):
        ...
        ret = self.event_channels.master_event.fire_event(
    ...
    def end_batch(self):
        ...
        self.event_channels.master_event.fire_event(
    ...
    def close_safe(self):
        if self.event_channels is not None:
            self.event_channels.unsubscribe(None, None, id(self))
            self.event_channels.unuse(id(self))
            self.event_channels = None
            _destroy_unused_shared_event_channels()
        self.event = None
