...
PYTHON_CMDS="/var/tmp/venv-salt-minion/bin/python3 python3 /usr/libexec/platform-python python27 python2.7 python26 python2.6 python2 python"
...
class Single:
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.venv_dir = "/var/tmp/venv-salt-minion"
...
ARGS = {arguments}\n'''.format(
    ...
    def execute_script(self, script, extension="py", pre_dir="", script_args=None):
        ...
        args = ""
        if script_args:
            args = " {}".format(" ".join(script_args) if isinstance(script_args, (list, tuple)) else script_args)
        if extension == "ps1":
            ret = self.shell.exec_cmd('"powershell {}"'.format(script))
        else:
            if not self.winrm:
                ret = self.shell.exec_cmd("/bin/sh '{}{}'{}".format(pre_dir, script, args))
            else:
