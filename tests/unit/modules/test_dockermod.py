...
class DockerUnitTestCase(TestCase, LoaderModuleMockMixin):
    ...
    def test_trans_tar_should_have_grains_in_sls_opts_including_pillar_override(self):
        ...
        patch_trans_tar = patch(
            "salt.modules.dockermod._prepare_trans_tar", fake_trans_tar,
        )
        patch_call = patch(
...
@skipIf(
    docker_mod.HAS_DOCKER_PY is False,
    "docker-py must be installed to run these tests. Skipping.",
)
class DockerTestCase(TestCase, LoaderModuleMockMixin):
    ...
    def test_ps_with_host_true(self):
        ...
        with patch.dict(
            docker_mod.__salt__, {"network.interfaces": network_interfaces}
        ):
            with patch.object(docker_mod, "_get_client", get_client_mock):
    ...
    def test_prune(self):
        ...
        self.assertEqual(
            ...
            [call("prune_containers", filters={}), call("prune_images", filters={})],
        )
    ...
    def test_port(self):
        ...
        with patch.object(docker_mod, "list_containers", list_mock), patch.object(
            ...
            self.assertEqual(
                ret, {"bar": {"6666/tcp": ports["bar"]["6666/tcp"]}, "baz": {}}
            )
            ret = docker_mod.port("ba?", private_port="6666/tcp")
            self.assertEqual(
                ret, {"bar": {"6666/tcp": ports["bar"]["6666/tcp"]}, "baz": {}}
