...
class TestMsgpack(TestCase):
    ...
    def test_buffered_base_unpack(self):
        self._test_buffered_base(pack_func=msgpack.pack, unpack_func=salt.utils.msgpack.unpack)
    ...
    def _test_unpacker_ext_hook(self, pack_func, **kwargs):
        ...
        self.assertEqual(unpacker.unpack(), {'a': salt.utils.msgpack.ExtType(2, b'321')})

    def _check(self, data, pack_func, unpack_func, use_list=False, strict_map_key=False):
        my_kwargs = {}
        if salt.utils.msgpack.version >= (0, 6, 0):
    ...
    def _test_pack_unicode(self, pack_func, unpack_func):
        ...
        for td in test_data:
            ...
            packer = salt.utils.msgpack.Packer()
            data = packer.pack(td)
            ret = salt.utils.msgpack.Unpacker(BytesIO(data), use_list=True, **raw).unpack()
            self.assertEqual(ret, td)
    ...
    def _test_ignore_errors_pack(self, pack_func, unpack_func):
        ret = unpack_func(
            pack_func(u'abc\uDC80\uDCFFdef', use_bin_type=True, unicode_errors='ignore'), use_list=True, **raw
        )
        self.assertEqual(u'abcdef', ret)
    ...
    @skipIf(salt.utils.msgpack.version < (0, 2, 2), 'use_single_float was added in msgpack==0.2.2')
    def _test_pack_float(self, pack_func, **kwargs):
        self.assertEqual(b'\xca' + struct.pack(str('>f'), 1.0), pack_func(1.0, use_single_float=True))
        self.assertEqual(b'\xcb' + struct.pack(str('>d'), 1.0), pack_func(1.0, use_single_float=False))
    ...
    def _test_odict(self, pack_func, unpack_func):
        ...
        self.assertEqual(seq, unpack_func(pack_func(od), object_pairs_hook=pair_hook, use_list=True))
    ...
    @skipIf(salt.utils.msgpack.version < (0, 6, 0), 'getbuffer() was added to Packer in msgpack 0.6.0')
    ...
    def test_binary_function_compatibility(self):
        ...
        if salt.utils.msgpack.dumps is not salt.utils.msgpack.packb:
            functions.append({'pack_func': salt.utils.msgpack.dumps, 'unpack_func': msgpack.unpackb})
        if salt.utils.msgpack.loads is not salt.utils.msgpack.unpackb:
            functions.append({'pack_func': msgpack.packb, 'unpack_func': salt.utils.msgpack.loads})
        ...
        for test_func in test_funcs:
            ...
            vanilla_run = self.no_fail_run(test_func, **{'pack_func': msgpack.packb, 'unpack_func': msgpack.unpackb})

            for func_args in functions:
                func_name = func_args['pack_func'] if func_args['pack_func'].__module__.startswith('salt.utils') \
                ...
                if hasattr(TestCase, 'subTest'):
                    with self.subTest(test=test_func.__name__, func=func_name.__name__):
                        ...
                        if run:
                            if str(vanilla_run) == str(run):
                                self.skipTest('Failed the same way as the vanilla msgpack module:\n{}'.format(run))
                else:
                    ...
                    if run:
                        # If the vanilla msgpack module errored, then skip if we got the same error
                        if str(vanilla_run) == str(run):
                            self.skipTest('Test failed the same way the vanilla msgpack module fails:\n{}'.format(run))
                        else:
