...
def salt_api_handler(io_loop, app_mock):
    ...
    handler = saltnado.SaltAPIHandler(app_mock, app_mock)
    # BaseSaltAPIHandler.initialize() builds a LocalClient, whose SaltEvent
    # creates a tornado IOLoop that calls asyncio.set_event_loop().
    # Every Future then attaches to salt's private loop
    # which makes tests using `io_loop.run_sync` timeout
    # as future's done callbacks are never fired on `io_loop` IOLoop
